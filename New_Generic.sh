...
echo "════════════════════════════════════════════════"
echo "🚀 ROM Setup & Build Fix Script"
...
for cmd in "${clones[@]}"; do
    ...
    # 5️⃣ Replace 'lineage' in .mk files
    ...
    # 7️⃣ KernelSU setup (only for kernel repo)
    if [ "$folder" == "kernel/nothing/sm7325" ]; then
        ...
        if [ "$DRY_RUN" -eq 1 ]; then
            ...
            # Remove existing KernelSU-Next if present
            if [ -d "KernelSU-Next" ]; then
                echo "Removing existing KernelSU-Next..."
                rm -rf KernelSU-Next
            fi
...
echo ""
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo -e "${BLUE}🔧 PHASE 2: Build Fixes${NC}"
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo ""

# ==============================================================================
# FIX 1: Remove ViPER4AndroidFX Dependency
# ==============================================================================
echo "──────────────────────────────────────────────"
echo "🎵 Fix 1: Removing ViPER4AndroidFX Dependency"
echo "──────────────────────────────────────────────"

DEVICE_MK="device/nothing/Spacewar/device.mk"

if [ -f "$DEVICE_MK" ]; then
    if [ "$DRY_RUN" -eq 1 ]; then
        echo "Would backup and comment out ViPER4AndroidFX in $DEVICE_MK"
    else
        # Backup original file
        if [ ! -f "${DEVICE_MK}.bak" ]; then
            cp "$DEVICE_MK" "${DEVICE_MK}.bak"
            echo -e "${GREEN}✓${NC} Backed up $DEVICE_MK"
        fi
        
        # Comment out ViPER4AndroidFX line
        if grep -q "packages/apps/ViPER4AndroidFX/config.mk" "$DEVICE_MK"; then
            sed -i 's|^\(.*packages/apps/ViPER4AndroidFX/config.mk.*\)|# \1|g' "$DEVICE_MK"
            echo -e "${GREEN}✓${NC} Commented out ViPER4AndroidFX dependency"
        else
            echo -e "${YELLOW}⚠${NC}  ViPER4AndroidFX not found in device.mk (may already be removed)"
        fi
    fi
else
    echo -e "${YELLOW}⚠${NC}  $DEVICE_MK not found"
fi

echo ""

# ==============================================================================
# FIX 2: Remove All SHA1 Hash Checks
# ==============================================================================
echo "──────────────────────────────────────────────"
echo "🔐 Fix 2: Removing SHA1 Hash Checks"
echo "──────────────────────────────────────────────"

VENDOR_MK="vendor/nothing/Spacewar/Android.mk"

if [ -f "$VENDOR_MK" ]; then
    if [ "$DRY_RUN" -eq 1 ]; then
        echo "Would backup and comment out SHA1 checks in $VENDOR_MK"
    else
        # Backup original file
        if [ ! -f "${VENDOR_MK}.bak" ]; then
            cp "$VENDOR_MK" "${VENDOR_MK}.bak"
            echo -e "${GREEN}✓${NC} Backed up $VENDOR_MK"
        fi
        
        # Comment out all add-radio-file-sha1-checked lines
        if grep -q "add-radio-file-sha1-checked" "$VENDOR_MK"; then
            sed -i 's|^\(.*add-radio-file-sha1-checked.*\)|# \1|g' "$VENDOR_MK"
            SHA1_COUNT=$(grep -c "# .*add-radio-file-sha1-checked" "$VENDOR_MK" || echo "0")
            echo -e "${GREEN}✓${NC} Commented out $SHA1_COUNT SHA1 hash checks"
        else
            echo -e "${YELLOW}⚠${NC}  No SHA1 checks found (may already be removed)"
        fi
    fi
else
    echo -e "${YELLOW}⚠${NC}  $VENDOR_MK not found"
fi

echo ""

# ==============================================================================
# FIX 3: Fix libhwbinder Visibility
# ==============================================================================
echo "──────────────────────────────────────────────"
echo "📚 Fix 3: Fixing libhwbinder Visibility"
echo "──────────────────────────────────────────────"

LIBHWBINDER_BP="system/libhwbinder/Android.bp"

if [ -f "$LIBHWBINDER_BP" ]; then
    if [ "$DRY_RUN" -eq 1 ]; then
        echo "Would backup and fix libhwbinder visibility in $LIBHWBINDER_BP"
    else
        # Backup original file
        if [ ! -f "${LIBHWBINDER_BP}.bak" ]; then
            cp "$LIBHWBINDER_BP" "${LIBHWBINDER_BP}.bak"
            echo -e "${GREEN}✓${NC} Backed up $LIBHWBINDER_BP"
        fi
        
        # Check if already public
        if grep -A 5 'name: "libhwbinder"' "$LIBHWBINDER_BP" | grep -q 'visibility: \["//visibility:public"\]'; then
            echo -e "${YELLOW}⚠${NC}  libhwbinder already has public visibility"
        else
            # Use sed for simple replacement
            sed -i '/name: "libhwbinder"/,/^}/ s|visibility: \[.*\]|visibility: ["//visibility:public"]|' "$LIBHWBINDER_BP"
            echo -e "${GREEN}✓${NC} Updated libhwbinder visibility to public"
        fi
    fi
else
    echo -e "${YELLOW}⚠${NC}  $LIBHWBINDER_BP not found"
fi

echo ""

# ==============================================================================
# FIX 4: Fix libhidltransport Visibility
# ==============================================================================
echo "──────────────────────────────────────────────"
echo "📚 Fix 4: Fixing libhidltransport Visibility"
echo "──────────────────────────────────────────────"

LIBHIDL_BP="system/libhidl/Android.bp"

if [ -f "$LIBHIDL_BP" ]; then
    if [ "$DRY_RUN" -eq 1 ]; then
        echo "Would backup and fix libhidltransport visibility in $LIBHIDL_BP"
    else
        # Backup original file
        if [ ! -f "${LIBHIDL_BP}.bak" ]; then
            cp "$LIBHIDL_BP" "${LIBHIDL_BP}.bak"
            echo -e "${GREEN}✓${NC} Backed up $LIBHIDL_BP"
        fi
        
        # Check if already public
        if grep -A 5 'name: "libhidltransport"' "$LIBHIDL_BP" | grep -q 'visibility: \["//visibility:public"\]'; then
            echo -e "${YELLOW}⚠${NC}  libhidltransport already has public visibility"
        else
            # Use sed for simple replacement
            sed -i '/name: "libhidltransport"/,/^}/ s|visibility: \[.*\]|visibility: ["//visibility:public"]|' "$LIBHIDL_BP"
            echo -e "${GREEN}✓${NC} Updated libhidltransport visibility to public"
        fi
    fi
else
    echo -e "${YELLOW}⚠${NC}  $LIBHIDL_BP not found"
fi

echo ""

# ==============================================================================
# FIX 5: Fix Duplicate google.xml Conflict
# ==============================================================================
echo "──────────────────────────────────────────────"
echo "📄 Fix 5: Fixing Duplicate google.xml Conflict"
echo "──────────────────────────────────────────────"

ADDONS_CONFIG="vendor/addons/config.mk"

if [ -f "$ADDONS_CONFIG" ]; then
    if [ "$DRY_RUN" -eq 1 ]; then
        echo "Would backup and comment out duplicate google.xml in $ADDONS_CONFIG"
    else
        # Backup original file
        if [ ! -f "${ADDONS_CONFIG}.bak" ]; then
            cp "$ADDONS_CONFIG" "${ADDONS_CONFIG}.bak"
            echo -e "${GREEN}✓${NC} Backed up $ADDONS_CONFIG"
        fi
        
        # Comment out google.xml line
        if grep -q "vendor/addons/prebuilt/product/etc/sysconfig/google.xml" "$ADDONS_CONFIG"; then
            sed -i 's|^\(.*vendor/addons/prebuilt/product/etc/sysconfig/google.xml.*\)|# \1|g' "$ADDONS_CONFIG"
            echo -e "${GREEN}✓${NC} Commented out duplicate google.xml entry"
        else
            echo -e "${YELLOW}⚠${NC}  google.xml entry not found (may already be removed)"
        fi
    fi
else
    echo -e "${YELLOW}⚠${NC}  $ADDONS_CONFIG not found (vendor/addons may not exist)"
fi

echo ""

# ==============================================================================
# CLEANUP: Remove build cache
# ==============================================================================
echo "──────────────────────────────────────────────"
...
if [ "$DRY_RUN" -eq 1 ]; then
    echo "Would remove out/ directory"
else
    if [ -d "out" ]; then
        ...
        echo -e "${YELLOW}⚠${NC}  out/ directory not found (already clean)"
    fi
fi
...
echo "════════════════════════════════════════════════"
echo ""
echo -e "${GREEN}Phase 1: Repository Setup${NC}"
echo "  ✓ Cloned device, kernel, vendor, and hardware repos"
echo "  ✓ Renamed files from lineage to $ROM_NAME"
echo "  ✓ Cherry-picked upstream commits"
echo "  ✓ Updated package allowlist"
echo "  ✓ Fixed vendor path references"
echo "  ✓ Downloaded device_framework_matrix.xml"
echo ""
echo -e "${GREEN}Phase 2: Build Fixes${NC}"
echo "  ✓ Removed ViPER4AndroidFX dependency"
echo "  ✓ Disabled SHA1 hash checks for radio blobs"
echo "  ✓ Fixed libhwbinder visibility to public"
echo "  ✓ Fixed libhidltransport visibility to public"
echo "  ✓ Removed duplicate google.xml entry"
echo "  ✓ Cleaned build cache"
echo ""
if [ "$DRY_RUN" -eq 0 ]; then
    echo -e "${BLUE}Backup files created:${NC}"
    echo "  • device/nothing/Spacewar/device.mk.bak"
    echo "  • vendor/nothing/Spacewar/Android.mk.bak"
    echo "  • system/libhwbinder/Android.bp.bak"
    echo "  • system/libhidl/Android.bp.bak"
    echo "  • vendor/addons/config.mk.bak (if exists)"
    echo ""
fi
